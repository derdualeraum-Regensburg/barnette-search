...
\title{Quantitative Hamiltonian Edge Separation\\Across a Three-Edge Cut}
\author{Thomas Bauer\\\small Research note for the Barnette Search project}
\date{Draft of September 17, 2026}
\begin{document}
\maketitle
\begin{abstract}
We give a constructive bound for Hamiltonian strong edge separation under
vertex splicing of cubic graphs. Separating cycle families with three-state
profiles $(a_1,a_2,a_3)$ and $(b_1,b_2,b_3)$ yield a separating family of size
$\sum_i a_i b_i$. Smaller edge-covering subfamilies within each state give the
refinement $\sum_i(p_i b_i+a_i q_i-p_i q_i)$. The result applies to arbitrary
simple cubic graphs with finite separation number, including Barnette graphs
outside the double-ladder family. It is a quantitative consequence of the
same gluing mechanism underlying the known qualitative $H^{+-}$ decomposition
theorem. Priority for the quantitative bounds is not asserted.
\end{abstract}

\section{Setting and relation to existing work}
A family $\mathcal F$ of Hamiltonian cycles strongly separates the edges of
$G$ if, for every ordered pair of distinct edges $(e,f)$, some $C\in\mathcal F$
contains $e$ and avoids $f$. Write $\hsep(G)$ for the minimum family size,
with value $\infty$ if no such family exists. Cycles are undirected edge sets;
families contain no repetitions.
...
3-connected bipartite graphs \cite[Lemma 4.5(iv) and Theorem 1.7(iv)]{gsw}.
That property is exactly the assertion $\hsep(G)<\infty$. The present note
counts explicit glued cycles and then reduces the number of selected pairs.
It does not claim a new qualitative decomposition theorem. The proof below
is self-contained and needs neither planarity nor bipartiteness.

\section{The quantitative gluing lemma}
...
\section{Consequences and explicit scope}
...
In particular, it does not prove extremal continuation or Barnette's conjecture.
...
The directory \texttt{artifacts/splice\_lemma\_20260917/} contains a separate
constructor and a standard-library-only verifier. The constructor enumerates
cycles by path search. The verifier independently enumerates perfect matchings,
retaining exactly those whose complements are connected spanning 2-factors.
It checks the input families, splice graph, state covers, every emitted cycle,
all ordered edge pairs, pair provenance, and both cardinality formulas.

The audit checks all six terminal bijections for five input pairs and also
proper separating subfamilies for the nested example, giving 36 labelled
examples. They are not asserted to be 36 different isomorphism classes.
The largest example has 30 vertices; no census reconstruction is started.
The regression suite has 29 tests, including rejection of damaged cycles,
empty state covers, and a Hamiltonian graph lacking strong separation.

Let $P_r=C_r\square K_2$ be the prism on $2r$ vertices, and let $S$ be the
splice of two cubes. For the natural terminal matching, representative bounds
are as follows. The input sizes are sizes of supplied families, not asserted
optimal values.
\begin{center}
\begin{tabular}{lrrrr}
\toprule
Factors & Output order & Input sizes & Full product & Refined\\
\midrule
$P_4,P_4$ &14&$6,6$&12&12\\
$P_4,P_6$ &18&$6,8$&16&16\\
$P_6,P_6$ &22&$8,8$&24&23\\
$P_8,P_8$ &30&$10,10$&44&40\\
$S,P_4$ &20&$10,6$&20&20\\
\bottomrule
\end{tabular}
\end{center}
The first four output orders are $2$ modulo $4$, so these Barnette graphs
are outside the odd-parameter double-ladder family in the main manuscript.
The bounds in the last two columns are only upper certificates. The symbolic
proof of Lemma~\ref{lem:splice} does not depend on these finite checks.

\paragraph{Reproduction and status.}
Regenerate the examples from the repository root with:
\begin{quote}\small\ttfamily
python artifacts/splice\_lemma\_20260917/build\_examples.py
\end{quote}
The adjacent \texttt{verify.py} checks any example JSON without installing
project dependencies. The report records
commands, source and certificate hashes, versions, runtime, and the absence
of random choices. This is an unreviewed research note; its quantitative
priority has not been established by an exhaustive literature review or
expert confirmation. The author remains responsible for the claims.
The note was developed with AI assistance for derivation, drafting and checks.
...
Author manuscript: \url{https://arxiv.org/abs/2202.11641}.
\end{thebibliography}
\end{document}
