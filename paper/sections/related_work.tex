...
separation by unrestricted paths or cycles are therefore not new. In
particular, the qualitative assertion $\hsep(G)<\infty$ is the known
$H^{+-}$ property. Its preservation under tight-cut decomposition is proved
by \citet[Lemma 4.5(iv) and Theorem 1.7(iv)]{GorskySteinerWiederrecht2023}.
Section~\ref{sec:quantitative-splice} counts the cycles used by this gluing
mechanism and reduces the selected pairs using statewise edge covers.
The present focus is the minimum size of a separating Hamiltonian family;
no absolute novelty or priority claim is made for the parameter or the
quantitative bounds.
